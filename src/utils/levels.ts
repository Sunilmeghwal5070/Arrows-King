 ...
 */
function getPatternScore(x: number, y: number, patternIndex: number, W: number, H: number): boolean {
  const cx = W / 2;
  const cy = H / 2;
  ...
  switch (patternIndex) {
    ...
    case 4: // The Symmetrical Windmill
      {
        ...
        if (x >= cx && y <= cy) return x === Math.floor(cx) || y === Math.floor(cy) - 2;
        if (x >= cx && y >= cy) return y === Math.floor(cy) || x === Math.floor(cx) + 2;
        if (x <= cx && y >= cy) return x === Math.floor(cx) || y === Math.floor(cy) + 2;
        if (x <= cx && y <= cy) return y === Math.floor(cy) || x === Math.floor(cx) - 2;
        return false;
      }
    ...
    case 5: // Isometric 3D Cube
      {
        ...
        const onInteriorY = (dx < 0.6 && y >= cy) || (Math.abs(dy - dx) < 0.6 && y <= cy);
    ...
    case 8: // Bento Grid Box
      return x === 2 || x === W - 2 || y === 2 || y === H - 2 || Math.abs(x - cx) < 0.6 || Math.abs(y - cy) < 0.6;

    case 9: // Symmetrical Star Outline
      return Math.abs(x - cx) < 0.6 || Math.abs(y - cy) < 0.6 || Math.abs(dx - dy) < 0.6;
    ...
    case 13: // Greek Key Frame
      return x === 2 || x === W - 2 || y === 2 || y === H - 2 || (x === 4 && y >= 4 && y <= H-4) || (y === 4 && x >= 4 && x <= W-4);

    case 14: // Symmetrical Butterfly
      return Math.abs(x - cx) < 0.6 || (dx > 1.2 && dx < W * 0.45 && dy < H * 0.4);

    case 15: // Marine Anchor Emblem
      {
        const topCircle = Math.abs(Math.sqrt(dx * dx + (y - 3) * (y - 3)) - 1.5) < 0.6;
    ...
    case 18: // Infinity Loop Figure-8
      {
        ...
        const cx1 = cx - 1.8;
        const cx2 = cx + 1.8;
        const d1 = Math.sqrt((x - cx1) * (x - cx1) + (y - cyCenter) * (y - cyCenter));
        const d2 = Math.sqrt((x - cx2) * (x - cx2) + (y - cyCenter) * (y - cyCenter));
    ...
    case 20: // Letter-S Snake Curve
      if (y === 2 || y === Math.floor(cy) || y === H - 2) return x >= 2 && x <= W - 2;
    ...
    case 23: // Spider-Web Concentrics
      return Math.abs(x - cx) < 0.6 || Math.abs(y - cy) < 0.6 || Math.abs(dx - dy) < 0.6 || x === 2 || x === W - 2 || y === 2 || y === H - 2;
    ...
    case 25: // Sea Trident Fork
      if (y === 2) return x === 2 || x === Math.floor(cx) || x === W - 2;
      if (y >= 2 && y <= cy) return x === 2 || x === Math.floor(cx) || x === W - 2;
    ...
    case 26: // Yin-Yang Balance
      {
        const onOuterCircle = Math.abs(Math.sqrt(dx*dx + dy*dy) - (cx - 1)) < 0.6;
        const sCurve = y <= cy ? Math.abs(x - (cx - Math.sin((y - 2) / (cy - 2) * Math.PI) * 1.5)) < 0.8 : Math.abs(x - (cx + Math.sin((y - cy) / (cy - 2) * Math.PI) * 1.5)) < 0.8;
        return onOuterCircle || sCurve;
      }

    case 27: // The Keyhole Outline
      {
        const topRing = Math.abs(Math.sqrt(dx * dx + (y - 3) * (y - 3)) - 1.5) < 0.6;
        const trapezoidLeft = Math.abs(x - (cx - (y - 4) * 0.5)) < 0.6 && y >= 4 && y <= H-2;
        const trapezoidRight = Math.abs(x - (cx + (y - 4) * 0.5)) < 0.6 && y >= 4 && y <= H-2;
    ...
    case 29: // Pixelated Space Invader
      ...
      if (x === 4 || x === W - 4) return y === 2 || y === 3 || y === 4 || y === 6;
      return Math.abs(x - cx) < 0.6 && (y === 2 || y === 3 || y === 5);

    case 30: // Diagonal Stepping Stairs
      return Math.abs(x - y) < 0.8 || Math.abs(x - y - 2) < 0.8 || Math.abs(x - y + 2) < 0.8;
    ...
    case 32: // Concentric Target Rings
      return x === 2 || x === W - 2 || y === 2 || y === H - 2 || x === 4 || x === W - 4 || y === 4 || y === H - 4;
    ...
    case 35: // Letter-H Bridge Support
      return x === 2 || x === W - 2 || (y === Math.floor(cy) && x >= 2 && x <= W - 2);
    ...
    case 38: // Symmetric Twin Moons
      {
        const moon1 = Math.abs(Math.sqrt((x - 1) * (x - 1) + dy * dy) - 2.5) < 0.6;
        const moon2 = Math.abs(Math.sqrt((x - (W - 1)) * (x - (W - 1)) + dy * dy) - 2.5) < 0.6;
        return moon1 || moon2;
      }
    ...
    case 40: // Twin Crossing Gates
      return Math.abs(x - cx + 1.5) < 0.6 || Math.abs(x - cx - 1.5) < 0.6 || Math.abs(y - cy + 1.5) < 0.6 || Math.abs(y - cy - 1.5) < 0.6;

    case 41: // Diamond Ring Jewel
      {
        const ring = Math.abs(Math.sqrt(dx * dx + (y - (cy + 1)) * (y - (cy + 1))) - 2.0) < 0.6;
    ...
    case 43: // Spiked Sunburst Wheel
      {
        ...
        const spikes = (x === 1 || x === W - 1) && Math.abs(y - cy) < 0.6 || (y === 1 || y === H - 1) && Math.abs(x - cx) < 0.6;
        return wheel || spikes;
      }

    case 44: // Chevron Arrow Stacks
      return Math.abs(y - x) < 0.6 || Math.abs(y - x - 2) < 0.6 || Math.abs(y - x + 2) < 0.6;
    ...
    case 48: // Shield of Protection Cross
      {
        const shieldBorder = (y === 2 && dx <= cx - 1) || ((x === 2 || x === W - 2) && y <= cy) || (Math.abs(y - (cy + dx * (H - 2 - cy) / (cx - 2))) < 0.8 && y >= cy);
        const centralCross = Math.abs(x - cx) < 0.6 && y >= 3 && y <= H - 3 || Math.abs(y - cy) < 0.6 && x >= 3 && x <= W - 3;
    ...
    case 51: // Symmetric Grid Maze
      return x === 2 || x === W - 2 || y === 2 || y === H - 2 || (x % 3 === 0 && y % 3 !== 0) || (y % 3 === 0 && x % 3 !== 0);
    ...
    case 53: // Royal Crown Scepter
      return y === 2 || y === H - 2 || x === 2 || x === W - 2 || Math.abs(x - cx) < 0.6;
    ...
    case 55: // Retro Space Rocket Body
      {
        ...
        const fuselage = y >= 3 && y <= H-4 && (x === Math.floor(cx - 1.5) || x === Math.floor(cx + 1.5));
...
function generatePatternedLevel(levelNumber: number): LevelConfig {
  ...
  const patternIndex = ((levelNumber - 1) % SHAPE_PATTERNS.length + SHAPE_PATTERNS.length) % SHAPE_PATTERNS.length;
  ...
  while (triesLeft > 0) {
    ...
    // Determine target bends (number of corners). Compact bends prevent trapping dense arrows!
    let minBends = 0;
    let maxBends = 2;
    if (levelNumber >= 6) {
      maxBends = 3;
    ...
    const targetBends = minBends + Math.floor(random() * (maxBends - minBends + 1));
    ...
    for (let segmentIndex = 0; segmentIndex < targetBends + 1; segmentIndex++) {
      ...
      allowedDirs.sort((dirA, dirB) => {
        ...
        for (let step = 1; step <= 3; step++) {
          ...
          if (patternTargetCells.has(kA) && !occupiedCells.has(kA) && !pathCells.has(kA)) {
            scoreA += 2;
          }
          const kB = `${prev.x + deltaB.x * step},${prev.y + deltaB.y * step}`;
          if (patternTargetCells.has(kB) && !occupiedCells.has(kB) && !pathCells.has(kB)) {
            scoreB += 2;
          }
        }
      ...
      for (const dir of allowedDirs) {
        // Longer segments for long winding snaking effect (segment lengths of 1 to 4)
        const maxLen = targetBends >= 5 ? 4 : 3;
        const targetLen = Math.floor(random() * maxLen) + 1;
        ...
        for (let currentLen = targetLen; currentLen >= 1; currentLen--) {
          ...
          for (let step = 1; step <= currentLen; step++) {
            ...
            if (!patternTargetCells.has(cellKey) || occupiedCells.has(cellKey) || pathCells.has(cellKey)) {
              currentOk = false;
              break;
            }
          ...
          if (currentOk && tempCells.length > 0) {
            ...
            while (rx >= 0 && rx <= gridWidth && ry >= 0 && ry <= gridHeight) {
              if (occupiedCells.has(`${rx},${ry}`) || pathCells.has(`${rx},${ry}`)) {
  ...
  if (finalPaths.length < Math.max(5, Math.floor(arrowCount * 0.65))) {
    return generateGuaranteedFailSafeLevel(levelNumber, arrowCount, gridWidth, gridHeight);
  }
...
function generateGuaranteedFailSafeLevel(
  ...
  while (triesLeft > 0 && paths.length < targetCount) {
    ...
    // Use compact snaking bends in the fallback level to fit target count perfectly
    let minBends = 0;
    let maxBends = 2;
    if (levelNumber >= 6) {
      maxBends = 3;
    ...
    for (let segmentIndex = 0; segmentIndex < targetBends + 1; segmentIndex++) {
      ...
      for (const dir of allowedDirs) {
        const maxLen = targetBends >= 4 ? 3 : 2;
        const targetLen = Math.floor(random() * maxLen) + 1;
        ...
        for (let currentLen = targetLen; currentLen >= 1; currentLen--) {
          ...
          for (let step = 1; step <= currentLen; step++) {
            ...
            if (testX < 1 || testX > gridW - 2 || testY < 1 || testY > gridH - 2) {
              currentOk = false;
              break;
            }
  ...
  if (finalPaths.length < 3) {
    const backupPaths: ArrowPath[] = [
      { id: makeId('fs-b-1'), points: [{ x: 1, y: 1 }, { x: 3, y: 1 }, { x: 3, y: 3 }], exitDirection: 'D', animState: { type: 'idle' } },
      { id: makeId('fs-b-2'), points: [{ x: 5, y: 1 }, { x: 5, y: 4 }, { x: 2, y: 4 }], exitDirection: 'L', animState: { type: 'idle' } },
      { id: makeId('fs-b-3'), points: [{ x: 1, y: 5 }, { x: 1, y: 2 }, { x: 4, y: 2 }], exitDirection: 'R', animState: { type: 'idle' } },
    ];
    return {
      levelNumber,
      title: 'Cosmic Flow',
    ...
    for (let segmentIndex = 0; segmentIndex < numBends + 1; segmentIndex++) {
      ...
      if (!relaxConstraint) {
        allowedDirs.sort((dirA, dirB) => {
          ...
          if (getPatternScore(testAx, testAy, patternIndex, gridWidth, gridHeight)) {
            scoreA = 1;
          }
          ...
          if (getPatternScore(testBx, testBy, patternIndex, gridWidth, gridHeight)) {
            scoreB = 1;
          }
      ...
      for (const dir of allowedDirs) {
        // Segment lengths: 1 to 2 looks compact, clean, and perfectly aligned
        const minLen = 1;
        const maxLen = numBends >= 2 ? 2 : 3;
        ...
        for (let step = 1; step <= len; step++) {
          ...
          if (testX < 1 || testX > gridWidth - 1 || testY < 1 || testY > gridHeight - 1) {
            isSegmentOk = false;
            break;
          }
 ...
 */
function topologicalClearingOrder(originalPaths: ArrowPath[], gridWidth: number, gridHeight: number): string[] {
  ...
  while (progress && activePaths.length > 0) {
    ...
    for (let i = 0; i < activePaths.length; i++) {
      ...
      const collision = checkCollision(arrow, activePaths, gridWidth, gridHeight);
 ...
 */
export function getSolvableArrows(paths: ArrowPath[], gridWidth: number, gridHeight: number): string[] {
 ...
 */
function doArrowsOverlap(p1a: Point, p1b: Point, p2a: Point, p2b: Point): boolean {
